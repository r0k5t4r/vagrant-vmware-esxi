...
module VagrantPlugins
  module ESXi
    module Action
      ...
      class CreateVM
        ...
        def createvm(env)
          ...
          if config.clone_from_vm.nil?
			...
			if config.vmkfstools == 'true'
			#if !config.clone_from_vm.nil? && config.vmkfstools == 'true'
			    puts "using vmkfstools for cloning..."
				nodisks_opt = "--noDisks"
				src_path = new_vmx_file
			end
          end
          ovf_cmd = "ovftool --noSSLVerify #{overwrite_opts} #{nodisks_opt} #{ovf_debug} "\
                "--acceptAllEulas "\
		  ...
		  #if !config.clone_from_vm.nil? && !config.vmkfstools.nil?
		  if !config.clone_from_vm.nil? && config.vmkfstools == 'true'
		  #puts ovf_cmd		
			puts "we are cloning using vmkfstools"
       Net::SSH.start(config.esxi_hostname, config.esxi_username,
            password:                   config.esxi_password,
            port:                       config.esxi_hostport,
            keys:                       config.local_private_keys,
      ...
      cmd = "test -d /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name} || mkdir /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name}"
      puts cmd
      r = ssh.exec!(cmd)
        if r.exitstatus != 0
              raise Errors::ESXiError,
                    message: "Unable to create dir."
            end
      #cmd = "test -f /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name}/#{desired_guest_name}.vmdk && rm -f /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name}/*vmdk*"
      cmd = "[ -f /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name}/#{desired_guest_name}.vmdk ] && rm -f /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name}/*vmdk* || echo 'old vmdk files do not exist.'"
      puts cmd
      r = ssh.exec!(cmd)
        if r.exitstatus != 0
              raise Errors::ESXiError,
                    message: "Unable to delete old vmdk files."
            end

      cmd = "vmkfstools -d thin -i /vmfs/volumes/#{@guestvm_dsname}/#{config.clone_from_vm}/#{config.clone_from_vm}.vmdk /vmfs/volumes/#{@guestvm_dsname}/#{desired_guest_name}/#{desired_guest_name}.vmdk"
      puts cmd
      r = ssh.exec!(cmd)
      puts r.exitstatus
            if r.exitstatus != 0
              raise Errors::ESXiError,
                    message: "Unable to clone vmx."
            end
      #sleep 10
    end
		  end
